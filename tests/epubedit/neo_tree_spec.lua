...
describe("neo-tree source", function()
  ...
  it("creates files under the current group using the OPF root prefix (EPUB2)", function()
    ...
    local opf_content = table.concat(vim.fn.readfile(core.state.current.opf), "\n")
    assert.is_truthy(opf_content:match('id="chapter2%.xhtml"'))
    assert.is_truthy(opf_content:match('href="Text/chapter2%.xhtml"'))
    assert.is_truthy(opf_content:match('<itemref idref="chapter2%.xhtml"'))
    create_stub:revert()
    input_stub:revert()
    core.cleanup(epubedit.get_config())
  end)
  ...
  it("creates files under the current group using the OPF root prefix (EPUB3)", function()
    ...
    assert.stub(create_stub).was_called_with(target_dir, match.is_function(), root_dir)
    local opf_content = table.concat(vim.fn.readfile(core.state.current.opf), "\n")
    assert.is_truthy(opf_content:match('id="nav%-new%.xhtml"'))
    assert.is_truthy(opf_content:match('href="Styles/nav%-new%.xhtml"'))
